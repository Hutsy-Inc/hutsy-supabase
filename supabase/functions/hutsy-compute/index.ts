...
// ---------------------------------------------------------------------------
/** 1) Remove every Plaid item from Plaid (stops billing) then wipe Supabase rows. */ // deno-lint-ignore no-explicit-any
async function deletePlaidData(db, userId) {
  const plaidEnv = (Deno.env.get('PLAID_ENV') ?? 'production').toLowerCase();
  const plaid = buildPlaidClient(plaidEnv);
  const { data: secrets } = await db.from('plaid_item_secrets').select('item_id, access_token').eq('user_id', userId).eq('plaid_env', plaidEnv);
...
/** 2) Cancel active Stripe subscriptions then delete the Stripe customer. */ // deno-lint-ignore no-explicit-any
async function deleteStripeData(db, userId) {
  const STRIPE_SECRET_KEY = Deno.env.get('STRIPE_SECRET_KEY');
  if (!STRIPE_SECRET_KEY) {
    console.warn('[delete_account] STRIPE_SECRET_KEY not set, skipping Stripe deletion');
    return;
  }
  const { data: profile } = await db.from('profiles').select('stripe_customer_id').eq('user_id', userId).maybeSingle();
  ...
  const subsRes = await fetch(`https://api.stripe.com/v1/subscriptions?customer=${encodeURIComponent(customerId)}&status=active&limit=10`, {
    headers
  });
  if (subsRes.ok) {
    const subsBody = await subsRes.json();
    for (const sub of subsBody?.data ?? []){
      const cancelRes = await fetch(`https://api.stripe.com/v1/subscriptions/${encodeURIComponent(sub.id)}`, {
        method: 'DELETE',
        headers
      });
      if (cancelRes.ok) {
  ...
  const delRes = await fetch(`https://api.stripe.com/v1/customers/${encodeURIComponent(customerId)}`, {
    method: 'DELETE',
    headers
  });
  if (delRes.ok) {
...
/** 3) Revoke all RevenueCat entitlements by deleting the subscriber. */ async function deleteRevenueCatData(userId) {
  ...
  const res = await fetch(`https://api.revenuecat.com/v1/subscribers/${encodeURIComponent(userId)}/delete`, {
    method: 'POST',
    headers: {
...
/** 4) Delete all remaining Supabase rows then hard-delete the Auth user. */ // deno-lint-ignore no-explicit-any
async function deleteSupabaseData(db, userId) {
  const tables = [
    'payments',
    'subscriptions',
    'chat_messages',
 ...
 *  Each service is wrapped independently — a failure in one never blocks the others. */ async function deleteAccount(userId) {
...
// ---------------------------------------------------------------------------
Deno.serve((req)=>AuthMiddleware(req, async (req)=>{
    if (req.method !== 'POST') {
      return json({
        ok: false,
